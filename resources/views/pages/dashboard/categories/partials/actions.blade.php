...
<div class="flex justify-end gap-2">
    <a href="{{ route('dashboard.categories.edit', $category) }}" class="{{ $iconButton }}"
    ...
    <x-ui.confirm-modal :action="route('dashboard.categories.destroy', $category)" method="DELETE"
        title="Delete Category"
        :description="__('Deleting this category will also remove any jobs linked to it.')"
        confirm-text="Delete" cancel-text="Cancel">
        <x-slot:trigger>
            <button type="button" class="{{ $dangerButton }}" aria-label="Delete {{ $category->name }}">
