                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select id="role" name="role" class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 dark:focus:ring-indigo-400/20 transition-all duration-200" required>
                            ...
                            <option value="{{ \App\Models\User::ROLE_ADMIN }}" {{ old('role', $user->role) === \App\Models\User::ROLE_ADMIN ? 'selected' : '' }}>👑 Admin - Full system access</option>
                            <option value="{{ \App\Models\User::ROLE_EMPLOYER }}" {{ old('role', $user->role) === \App\Models\User::ROLE_EMPLOYER ? 'selected' : '' }}>🏢 Employer - Can post and manage jobs</option>
                            <option value="{{ \App\Models\User::ROLE_CANDIDATE }}" {{ old('role', $user->role) === \App\Models\User::ROLE_CANDIDATE ? 'selected' : '' }}>👤 Candidate - Can apply for jobs</option>
                        </select>
                        @error('role')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
